            ...
            <div class="main-right-wrapper ms-auto">
                <div class="data-tab">
                        ...
                        <?php
                            ...
                            $getMark = mysqli_query($connect, "SELECT `work_mark` FROM `works` WHERE `user_fullname` = '$aga' AND `work_date` = '$today' ");
                            $mark = mysqli_fetch_row($getMark);
                            
                            if($ygy == 0) {
                              echo "<p class='non-load'> Вы еще не загрузили работу </p> ";
                            } else {
                              echo "<a download class='today-work' href='{$way[0]}'>Скачать свою загруженную работу</a><br>";
                              echo "<p>Оценка: {$mark[0]}</p>";
                            }
